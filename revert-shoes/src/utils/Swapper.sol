...
abstract contract Swapper is IUniswapV3SwapCallback, IErrors {
    ...
    function _routerSwap(
        ...
        if (
            params.amountIn != 0 &&
            params.swapData.length != 0 &&
            address(params.tokenOut) != address(0)
        ) {
            ...
            (address router, bytes memory routerData) = abi.decode(
                params.swapData,
                (address, bytes)
            );

            if (router == zeroxRouter) {
                ZeroxRouterData memory data = abi.decode(
                    routerData,
                    (ZeroxRouterData)
                );
                ...
                UniversalRouterData memory data = abi.decode(
                    routerData,
                    (UniversalRouterData)
                );
    ...
    function _poolSwap(
        ...
        if (params.amountIn > 0) {
            ...
            amountInDelta = params.swap0For1
                ? uint256(amount0Delta)
                : uint256(amount1Delta);
            amountOutDelta = params.swap0For1
                ? uint256(-amount1Delta)
                : uint256(-amount0Delta);
