...
contract AutoRange is Automator {
    ...
    function executeWithVault(
        ...
        IVault(vault).transform(
            params.tokenId,
            address(this),
            abi.encodeWithSelector(AutoRange.execute.selector, params)
        );
    }
    ...
    function _getTickSpacing(uint24 fee) internal view returns (int24) {
        if (fee == 10000) {
            ...
            int24 spacing = IUniswapV3Factory(factory).feeAmountTickSpacing(
                fee
            );
            if (spacing <= 0) {
