    ...
    IErrors
{
    ...
    mapping(address => mapping(uint256 => mapping(address => bool)))
        public transformApprovals; // owners permissions for other addresses to call transform on owners behalf (e.g. AutoRange contract)
    ...
    //can we use a flashloan to get cheaper deals of repayment?
    {
    ...
    function loanInfo(
        ...
        returns (
            ...
            // q what the diffence of the two below.
        ...
        )
    {
        ...
        bool isHealthy;
        (isHealthy, fullValue, collateralValue, ) = _checkLoanIsHealthy(
            tokenId,
            debt
        );

        if (!isHealthy) {
            (liquidationValue, liquidationCost, ) = _calculateLiquidation(
                debt,
                fullValue,
                collateralValue
            );
    ...
    /// @inheritdoc IERC4626
    // can this break the protocol by checking repayment with this function
    ...
    function convertToAssets(
        ...
        (, uint256 lendExchangeRateX96) = _calculateGlobalInterest();
        return
            _convertToAssets(shares, lendExchangeRateX96, Math.Rounding.Down);
    }
    ...
    function borrow(uint256 tokenId, uint256 assets) external override {
        bool isTransformMode = transformedTokenId > 0 &&
            transformedTokenId == tokenId &&
            transformerAllowList[msg.sender];
    ...
    function withdrawReserves(
        ...
        (uint256 balance, , uint256 reserves) = _getAvailableBalance(
            newDebtExchangeRateX96,
            newLendExchangeRateX96
        );
        ...
        //@audit why did you not update the global interest later
    ...
    function setLimits(
        ...
        emit SetLimits(
            ...
            _globalLendLimit,
            _globalDebtLimit,
            _dailyLendIncreaseLimitMin,
            _dailyDebtIncreaseLimitMin
        );
    }
    ...
    function setReserveProtectionFactor(
        uint32 _reserveProtectionFactorX32
    ) external onlyOwner {
    ...
    function setTokenConfig(
        ...
        tokenConfigs[token]
            .collateralValueLimitFactorX32 = collateralValueLimitFactorX32;
        emit SetTokenConfig(
            token,
            collateralFactorX32,
    ...
    function _deposit(
        // who is this receiver
        // can you deposit on behalf of someone
        address receiver,
        uint256 amount,
        bool isShare,
        ...
        // q check if revert when fails stops calling or it will continue to mint
        ...
        returns (uint256 balance, uint256 available, uint256 reserves)
    {
        ...
        // money in + money give if it is great than the money provided return money in + money given minus money give out
    ...
    function _sendPositionValue(
        ...
        if (liquidationValue == fullValue) {
            ...
            (, , , liquidity, , , fees0, fees1) = oracle.getPositionBreakdown(
                tokenId
            );
            ...
            if (liquidationValue < feeValue) {
                ...
                fees1 = type(uint128).max;
                liquidity = uint128(
                    ((liquidationValue - feeValue) * liquidity) /
                        (fullValue - feeValue)
                );
            }
        }
        ...
        )
    {
        ...
        if (fullValue >= maxPenaltyValue) {
            ...
            uint256 startLiquidationValue = (debt * fullValue) /
                collateralValue;
            uint256 penaltyFractionX96 = (Q96 -
                (((fullValue - maxPenaltyValue) * Q96) /
                    (startLiquidationValue - maxPenaltyValue)));
            ...
            uint256 penaltyValue = (fullValue *
    ...
    function _handleReserveLiquidation(
        ...
        (, , uint256 reserves) = _getAvailableBalance(
            newDebtExchangeRateX96,
            newLendExchangeRateX96
        );
        ...
        if (reserveCost > reserves) {
            ...
            uint256 totalLent = _convertToAssets(
                totalSupply(),
                newLendExchangeRateX96,
                Math.Rounding.Up
            );
            ...
            newLendExchangeRateX96 =
                ((totalLent - missing) * newLendExchangeRateX96) /
                totalLent;
